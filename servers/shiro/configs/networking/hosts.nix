...
with lib;
let
  networking = mapAttrs (netName: netCfg:
    let
      name = if netCfg.name != null
            then netCfg.name
            else netName;
    in
    netCfg // {
  ...
  hostToNameValPair = (host:
    ...
    nameValuePair ipAddrs.${if ipAddrs ? "clan" then "clan" else "elan"} (concatMap genNames names));
in
{
