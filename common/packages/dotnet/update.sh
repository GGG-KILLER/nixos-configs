...
" >&2
    ...
    for sem_version in "$@"; do
        echo "Generating ./${sem_version}.nix"
        ...
        # If it is, exit early
        if [ "$patch_specified" == false ] && [ -f "./${sem_version}.nix" ]; then
            current_version=$(nix-instantiate --eval -E "(import ./${sem_version}.nix { \
            buildAspNetCore = { ... }: {}; \
            buildNetRuntime = { ... }: {}; \
...
$(sdk_packages "${runtime_version}")
        ...
        cp "${result}" "./${sem_version}.nix"
        echo "Generated ./${sem_version}.nix"
    done
}
