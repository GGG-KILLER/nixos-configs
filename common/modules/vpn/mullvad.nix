...
in
{
  ...
  config = mkIf mullvadConfig.enable {
    ...
    systemd.tmpfiles.rules = [ "d '${TALPID_NET_CLS_MOUNT_DIR}' 0755 root root" ];
      ...
      serviceConfig =
        ...
        in
        {
          ...
          ExecStartPost = pkgs.writeShellScript "mullvad_setup" ''
            ...
            ${optionalString (
              mullvadConfig.location != null
            ) "${mullvad} relay set location ${mullvadConfig.location}"}
            ...
            ${optionalString (
              mullvadConfig.nameservers != [ ]
            ) "${mullvad} dns set custom ${concatStringsSep " " mullvadConfig.nameservers}"}

            ${optionalString (config.my.secrets.vpn.mullvad != null)
              "${mullvad} account login \"${
                builtins.replaceStrings [ " " ] [ "" ] config.my.secrets.vpn.mullvad.account
              }\""
            }
          '';
          Restart = "always";
