{ config, ... }:
{
  ...
  services.grafana = {
    ...
    settings = {
      server = {
        ...
        root_url = "https://grafana.izuna.lan/";
        enable_gzip = true;
      };
      security = {
      ...
      datasources = [
        {
          ...
          type = "prometheus";
          access = "proxy";
          url = "http://localhost:${toString config.izuna.ports.prometheus}";
          jsonData = {
            cacheLevel = "Low";
            defaultEditor = "code";
  ...
  services.caddy.virtualHosts."grafana.izuna.lan".extraConfig = ''
