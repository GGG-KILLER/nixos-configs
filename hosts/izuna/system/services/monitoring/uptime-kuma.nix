{ config, ... }:
{
  izuna.dynamic-ports = [ "uptime-kuma" ];

  services.uptime-kuma.enable = true;
  services.uptime-kuma.settings = {
    NODE_EXTRA_CA_CERTS = config.security.pki.caBundle;
    PORT = toString config.izuna.ports.uptime-kuma;
  };
  ...
  services.caddy.virtualHosts."status.izuna.lan".extraConfig = ''
    reverse_proxy 127.0.0.1:${toString config.izuna.ports.uptime-kuma}
  '';
}
