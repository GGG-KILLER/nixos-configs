...
}:
{
  ...
  virtualisation.oci-containers.containers.byparr = rec {
    ...
    ports = [ "${toString config.izuna.ports.byparr}:8191" ];
    extraOptions = [
      "--ipc=private"
    ];
  };
  ...
  services.suwayomi-server = {
    ...
    settings = {
      server.ip = "127.0.0.1";
      server.port = config.izuna.ports.suwayomi;
      ...
      server.flareSolverrUrl = "http://127.0.0.1:${toString config.izuna.ports.byparr}";
  ...
  services.caddy.virtualHosts = {
    "suwayomi.lan".extraConfig = ''
      reverse_proxy http://127.0.0.1:${toString config.izuna.ports.suwayomi}
    '';
    "byparr.suwayomi.lan".extraConfig = ''
      reverse_proxy http://127.0.0.1:${toString config.izuna.ports.byparr}
    '';
  };
}
