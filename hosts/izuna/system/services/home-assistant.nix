...
in
{
  ...
  virtualisation.oci-containers.containers.hass-z2m = rec {
    ...
    ports = [ "127.0.0.1:${toString config.izuna.ports.zigbee2mqtt}:8080" ];
    devices = [
  ...
  virtualisation.oci-containers.containers.hass = rec {
    ...
    dependsOn = [ "hass-mqtt" ];
    networks = [ "home-assistant" ];
    # ports = [ "127.0.0.1:${toString config.izuna.ports.home-assistant}:8123" ];
    ports = [ "${toString config.izuna.ports.home-assistant}:8123" ];
  ...
  networking.firewall.allowedTCPPorts = [ config.izuna.ports.home-assistant ];

  services.caddy.virtualHosts = {
    "hass.lan".extraConfig = ''
      reverse_proxy http://127.0.0.1:${toString config.izuna.ports.home-assistant}
    '';
    "z2m.hass.lan".extraConfig = ''
      reverse_proxy http://127.0.0.1:${toString config.izuna.ports.zigbee2mqtt}
    '';
  };
}
