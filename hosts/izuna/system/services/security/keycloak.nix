{ config, ... }:
{
  ...
  services.keycloak = {
    ...
    settings = {
      ...
      http-enabled = true;
      http-host = "127.0.0.1";
      http-port = config.izuna.ports.keycloak;
      proxy-headers = "xforwarded";
    };
    database = {
      createLocally = false;
      host = "localhost";
      port = config.izuna.ports.postgres;
      name = "keycloak";
      username = "keycloak";
  ...
  services.caddy.virtualHosts."sso.lan".extraConfig = ''
    reverse_proxy http://127.0.0.1:${toString config.izuna.ports.keycloak}
  '';
}
