{ pkgs, config, ... }:
{
  ...
  services.openbao.settings = {
    ...
    api_addr = "https://vault.izuna.lan";
    cluster_addr = "http://127.0.0.1:${toString config.izuna.ports.bao-cluster}";
    ...
    listener.default = {
      type = "tcp";
      tls_disable = "true";
      address = "127.0.0.1:${toString config.izuna.ports.bao}";
  ...
  services.caddy.virtualHosts."vault.izuna.lan".extraConfig = ''
    reverse_proxy http://127.0.0.1:${toString config.izuna.ports.bao}
  '';
}
