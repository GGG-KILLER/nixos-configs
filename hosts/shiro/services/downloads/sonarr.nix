{ lib, config, ... }:
{
  config = lib.mkIf (!config.cost-saving.enable || !config.cost-saving.disable-downloaders) {
    ...
    services.caddy.virtualHosts = {
      "sonarr.shiro.lan".extraConfig = "reverse_proxy http://127.0.0.1:${toString config.shiro.ports.sonarr}";
      "jackett.shiro.lan".extraConfig = "reverse_proxy http://127.0.0.1:${toString config.shiro.ports.jackett}";
    };
  };
}
