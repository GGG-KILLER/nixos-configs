...
in
{
  ...
  virtualisation.oci-containers.containers =
    ...
    in
    {
      ...
      authentik-server = {
        ...
        ports = [
          "${toString config.jibril.ports.authentik}:9000"
          "${toString config.jibril.ports.authentik-ssl}:9443"
        ];
  ...
  services.nginx.upstreams.authentik = {
    servers."127.0.0.1:${toString config.jibril.ports.authentik-ssl}" = { };
    extraConfig = ''
      keepalive 10;
    '';
  };

  modules.services.nginx.virtualHosts."sso.jibril.lan" = {
    ssl = true;
    locations."/" = {
